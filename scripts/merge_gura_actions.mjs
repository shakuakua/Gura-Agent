...
const SOURCE = 'public/smoller_gura_blender_actions.glb'
const TARGET = 'public/smoller_gura_-_gawr_gura_holomyth.glb'
const OUTPUT = 'public/smoller_gura_actions.glb'
...
function readGlb(path) {
  ...
  while (offset < buffer.length) {
    ...
    const chunkType = type.replace(/\0/g, '').trim()
    if (chunkType === 'JSON') {
...
function writeGlb(path, json, bin) {
  ...
  const total =
    12 + 8 + jsonBuffer.length + jsonPadding + 8 + bin.length + binPadding
...
for (const animation of source.json.animations) {
  if (!animation.name.startsWith('Gura_')) continue
